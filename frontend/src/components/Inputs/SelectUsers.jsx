...
const SelectUsers = ({selectedUsers, setSelectedUsers}) => {
    ...
    const toggleUserSelection = (uesrId) => {
        setTempSelectedUsers((prev) => 
            prev.includes(uesrId)
            ? prev.filter((id) => id !== uesrId)
            : [...prev, uesrId]
        )
    }
            ...
            >
                <div className='space-y-4 h-[60vh] overflow-y-auto'>
                    {allUsers.map((user) => (
                        <div
                            key={user._id}
                            className='flex items-center gap-4 p-3 border-b border-gray-200 cursor-pointer hover:bg-slate-50'
                        ...
                        >
                            ...
                            {tempSelectedUsers.includes(user._id) && (
                                <LuCheck className="text-lg text-primary" />
                            )}
                        </div>
                    ))}
                </div>
