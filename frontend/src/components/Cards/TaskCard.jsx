...
const TaskCard = ({
  ...
  completedTasks = todoChecklist.filter(item => item.completed).length
  const progressPercentage =
    todoChecklist?.length > 0
      ? (completedTasks / todoChecklist?.length) * 100
      : 0;
      ...
      >
        ...
        <p className="text-[13px] text-gray-700/80 font-medium mt-2 mb-2 leading-[18px]">
          ...
          <span className="font-semibold text-gray-700">
            {completedTasks} / {todoChecklist.length || 0}
          </span>
        </p>

        <Progress progress={progressPercentage} status={status} />
      </div>
      ...
      <div className="px-4">
        <div className="flex items-center justify-between my-1">
          <div>
            ...
            <p className="text-[13px] font-medium text-gray-900">{moment(createdAt).format("DD MMM YYYY")}</p>
          </div>

          <div>
            <label className="text-xs text-gray-500">Due Date</label>
            <p className="text-[13px] font-medium text-gray-900">{moment(dueDate).format("DD MMM YYYY")}</p>
          </div>
        </div>
